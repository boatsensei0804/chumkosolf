...
import { adviseeFormSchema, type AdviseeFormValues } from "./adviseeForm";

type AdviseeEditFormProps = {
  defaultValues: AdviseeFormValues;
  onSubmit: (values: AdviseeFormValues) => void;
  isSubmitting: boolean;
  errorMessage?: string;
};

type FieldName = Exclude<FieldPath<AdviseeFormValues>, "address">;

function TextField(props: {
  control: Control<AdviseeFormValues>;
  name: FieldName;
  label: string;
  error?: string;
  placeholder?: string;
...
export function AdviseeEditForm({
  defaultValues,
  onSubmit,
  isSubmitting,
  errorMessage,
}: AdviseeEditFormProps): ReactNode {
  const {
    control,
    handleSubmit,
    setValue,
    formState: { errors },
  } = useForm<AdviseeFormValues>({ resolver: zodResolver(adviseeFormSchema), defaultValues });
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5" noValidate>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 gap-x-4 gap-y-3 sm:grid-cols-2">
          <TextField control={control} name="prefix" label="คำนำหน้า" error={errors.prefix?.message} placeholder="เช่น เด็กชาย/เด็กหญิง" />
